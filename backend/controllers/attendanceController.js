  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "internshipId, partnerId, timetableDate and students[] are required.",
      });
    }

    // ── Block future dates 03-09-2026──────────────────────────────────────────────
    const sessionDate = moment(timetableDate).startOf("day");
    ...
    const schedule = await InternshipSchedule.findOne({
      internshipId,
      partnerId,
    });
    if (!schedule) {
    ...
    if (schedule.isClosed) {
      return res.status(403).json({
        success: false,
        message:
          "This internship schedule is closed. Attendance records are locked and cannot be modified.",
      });
    }
    ...
    for (const { studentId, isPresent } of students) {
      ...
      if (
        sessionType === "hybrid" &&
        existing?.onlineAttendance?.meetsThreshold === true
      ) {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "internshipId, studentId, timetableDate, partnerId, and isPresent are required.",
      });
    }
    //03-06-2026-------------
    const sessionDate = moment(timetableDate).startOf("day");
    ...
    if (schedule.isClosed) {
      return res.status(403).json({
        success: false,
        message:
          "This internship schedule is closed. Attendance records cannot be overridden after the schedule is closed.",
      });
    }

    let slot = schedule.timetable.find(
      (s) =>
        new Date(s.date).toDateString() ===
        new Date(timetableDate).toDateString(),
    );
    if (!slot && schedule.batches) {
      for (const batch of schedule.batches) {
        slot = batch.timetable.find(
          (s) =>
            new Date(s.date).toDateString() ===
            new Date(timetableDate).toDateString(),
        );
...
const getAttendanceDashboard = async (req, res) => {
  try {
    ...
    if (!internshipId) {
      return res
        .status(400)
        .json({ success: false, message: "internshipId is required." });
    }
    ...
    for (const o of acceptedOffers)
      offerMap[String(o.studentId)] = o.preferredTimeSlot;
    ...
    const users = await Userwebapp.find({ _id: { $in: studentIds } }).select(
      "name email",
    );
    const userMap = {};
    ...
    const students = studentIds.map((sid) => {
      ...
      if (schedule.batches && schedule.batches.length > 0 && studentBatch) {
        const matchedBatch = schedule.batches.find(
          (b) => b.timeSlot === studentBatch,
        );
        if (
          matchedBatch &&
          matchedBatch.timetable &&
          matchedBatch.timetable.length > 0
        ) {
      ...
      const percent =
        studentTotalSessions > 0
          ? Math.round((attended / studentTotalSessions) * 100)
          : 0;
    ...
    return res.status(200).json({
      ...
      onlineMinDurationMins:
        schedule.attendanceSettings?.onlineMinDurationMins ?? 0,
  ...
  try {
    ...
    if (
      ...
      if (preferredTimeSlot) {
        ...
        const matchedBatch = schedule.batches.find(
          (b) => b.timeSlot === preferredTimeSlot,
        );
    ...
    const records = await Attendance.find({ internshipId, studentId })
      .sort({ timetableDate: 1 })
      .lean();
    ...
    const issuedCert = await IssuedCertificate.findOne({
      internshipId,
      studentId,
    }).lean();
...
const updateAttendanceSettings = async (req, res) => {
  try {
    ...
    if (!internshipId || !partnerId) {
      return res.status(400).json({
        success: false,
        message: "internshipId and partnerId are required.",
      });
    }

    const existingSchedule = await InternshipSchedule.findOne({
      internshipId,
      partnerId,
    });
    if (existingSchedule?.isClosed) {
      return res.status(403).json({
        success: false,
        message:
          "This internship schedule is closed. Settings cannot be changed.",
      });
    }

    // ── Validate onlineMinDurationMins BEFORE updating - 03-09-2026 ──────────────────
    if (onlineMinDurationMins !== undefined) {
      const allSlots = [
        ...(existingSchedule.timetable || []),
        ...(existingSchedule.batches || []).flatMap((b) => b.timetable || []),
      ];

      const durations = allSlots
        .map((s) => {
      ...
      const maxAllowed = durations.length ? Math.min(...durations) : 240;

      if (onlineMinDurationMins > maxAllowed) {
        return res.status(400).json({
          success: false,
          message: `Online min duration cannot be greater than the shortest session (${maxAllowed} minutes).`,
        });
      }
    }
    // ────────────────────────────────────────────────────────────────────
    ...
    if (minAttendancePercent !== undefined)
      update["attendanceSettings.minAttendancePercent"] = minAttendancePercent;
    if (onlineMinDurationMins !== undefined)
      update["attendanceSettings.onlineMinDurationMins"] =
        onlineMinDurationMins;
    if (trackingEnabled !== undefined)
      update["attendanceSettings.trackingEnabled"] = trackingEnabled;
  ...
  try {
    ...
    if (schedule.isClosed)
      return res.status(403).json({
        success: false,
        message:
          "This internship schedule is closed. No new sessions can be started.",
      });
    ...
    if (type === "offline") {
      return res
        .status(400)
        .json({ success: false, message: "Offline sessions do not use OTP" });
    }
    ...
    if (now < sessionStart) {
      return res
        .status(400)
        .json({ success: false, message: "Session has not started yet" });
    }
    ...
    const sessionEndExpiry = moment
      .utc(slot.date)
      .utcOffset("+05:30")
      .set({ hour: endHour, minute: endMin + 15, second: 0, millisecond: 0 })
      .toDate();
    const minimumExpiry = new Date(now.getTime() + 15 * 60 * 1000);
    const expiresAt =
      sessionEndExpiry > minimumExpiry ? sessionEndExpiry : minimumExpiry;
    ...
    return res
      .status(200)
      .json({ success: true, otp, expiresAt, timetableDate });
  } catch (error) {
  ...
  try {
    ...
    if (!slot && schedule.batches) {
      if (preferredTimeSlot) {
        ...
        if (matchedBatch) {
          slot = matchedBatch.timetable.find(
            (s) =>
              new Date(s.date).toDateString() ===
              new Date(timetableDate).toDateString(),
          );
    ...
    if (!offer) {
      return res
        .status(403)
        .json({ success: false, message: "Not enrolled in this internship" });
    }
    ...
    if (existingRecord) {
      if (existingRecord.resolvedBy === "override") {
        return res.status(400).json({
          success: false,
          message: "Attendance has been manually overridden by the partner",
        });
      }

      if (
        existingRecord.isPresent ||
        existingRecord.onlineAttendance?.meetsThreshold ||
        existingRecord.onlineDetails?.joined
      ) {
        return res.status(200).json({
          success: true,
          message: "Already marked present for online portion",
        });
      }
    }
    ...
    if (type === "online") {
      ...
      newIsPresent =
        existingRecord.onlineAttendance?.meetsThreshold === true &&
        existingRecord.offlineAttendance?.markedPresent === true;
    }
    ...
    return res.status(200).json({
      success: true,
      message: isHybrid
        ? "Online portion marked. Pending offline attendance."
...
const endSession = async (req, res) => {
  try {
    ...
    if (schedule.isClosed)
      return res.status(403).json({
        success: false,
        message: "This internship schedule is closed.",
      });
    ...
    return res
      .status(200)
      .json({ success: true, message: "Session ended. OTP deactivated." });
  } catch (error) {
...
const getSessionStatus = async (req, res) => {
  try {
    ...
    const slot = studentTimetable.find(
      (s) =>
        s.sessionOtp?.isActive === true &&
        new Date() <= new Date(s.sessionOtp.expiresAt),
    );
