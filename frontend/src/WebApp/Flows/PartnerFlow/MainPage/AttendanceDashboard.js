...
export default function AttendanceDashboard({
  ...
  const [markOfflineModal, setMarkOfflineModal] = useState({
    open: false,
    timetableDate: null,
    students: [],
  });
  ...
  const [studentSummaryModal, setStudentSummaryModal] = useState({
    open: false,
    student: null,
  });
  ...
  // Helper functions 03-09-2026---------------------------
  const toLocalDateStr = (d) => {
    const date = new Date(d);
    return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
  };

  const getSessionDurationMins = (startTime, endTime) => {
  ...
  const getMaxOnlineDuration = () => {
    if (!dashboardData) return 240;

    const slots = [...(dashboardData.timetable || [])];

    const durations = slots
      .map((s) => getSessionDurationMins(s.startTime, s.endTime))
      .filter((d) => d > 0);
    // console.log("durations :", durations);
    return durations.length ? Math.min(...durations) : 240;
  };
  //----------------------------------------------------------------------------
    ...
    async (showLoading = true) => {
      ...
      try {
        const res = await axios.get(
          `/api/attendance/dashboard/${internshipId}`,
          {
            params: { partnerId },
            headers: {
  ...
  const handleReissueCerts = async () => {
    ...
    try {
      const res = await axios.post(
        "/api/schedule/reissue-certificates",
        {
          internshipId,
          partnerId,
        },
      ...
      setReissueResult({
        type: "error",
        message: err.response?.data?.error || "Failed to re-issue certificates",
      });
    } finally {
  ...
  const handleMarkOfflineSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "/api/attendance/mark-offline",
        {
          internshipId,
          partnerId,
          timetableDate: markOfflineModal.timetableDate,
          students: markOfflineModal.students.map((s) => ({
      ...
      //Add the alert message .message which comes from backend - 03-09-2026
      alert(
        err.response?.data?.message ||
          err.response?.data?.error ||
          "Failed to mark offline attendance",
      );
    }
  };
  ...
  const renderLeftPanel = () => {
    ...
    const activeBatchIndex =
      selectedBatch < batchKeys.length ? selectedBatch : 0;
    ...
    return (
      <div className="flex flex-col h-full overflow-hidden">
            ...
            <button
              ...
              onClick={() => {
                ...
                setSelectedTimetableDate(
                  newBatchSlots.length > 0 ? newBatchSlots[0].date : null,
                );
              }}
              className={`px-4 py-1.5 rounded-t-lg text-sm font-semibold border-b-2 transition-colors ${
            ...
            <tbody className="divide-y divide-gray-100">
              {activeBatchSlots.map((slot, idx) => {
                const isSelected =
                  ...
                  new Date(selectedTimetableDate).toDateString() ===
                ...
                const sessionActive =
                  hasBeenStarted &&
                  otpInfo?.isActive &&
                  new Date() <= new Date(otpInfo.expiresAt);
        ...
        {selectedTimetableDate &&
          (() => {
            const slot = activeBatchSlots.find(
              (s) =>
                new Date(s.date).toDateString() ===
                new Date(selectedTimetableDate).toDateString(),
            );
            const preciseSlot =
              activeBatchSlots.find(
                (s) =>
                  new Date(s.date).toDateString() ===
                    new Date(selectedTimetableDate).toDateString() &&
              ...
              <div className="mt-3 shrink-0">
                {isLocked && (
                  <div className="w-full py-2 px-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-sm font-medium flex items-center gap-2">
                    <span>🔒</span> Schedule closed — sessions cannot be
                    started.
                  </div>
                )}
                {!isLocked && !sessionActive && !sessionEnded && (
                  ...
                  >
                    ...
                    {startingSession
                      ? "Starting..."
                      : `Start Session — ${format(parseISO(resolvedSlot.date), "MMM d")} ${resolvedSlot.startTime}`}
                  </button>
                )}
                {sessionActive && (
                  <div className="bg-white rounded-lg border border-green-200 p-4 text-center shadow-inner relative overflow-hidden">
                    ...
                    <div className="text-3xl font-black text-gray-900 tracking-[0.2em] my-2 font-mono">
                      {otpInfo.code}
                    </div>
                    <p className="text-xs text-gray-600 mb-3 bg-gray-50 py-1 rounded-md inline-block px-3">
                      ...
                      />
                      Valid until{" "}
                      {format(parseISO(otpInfo.expiresAt), "h:mm a")}
                    </p>
                    {!isLocked && (
                      <button
                        onClick={() =>
  ...
  const renderRightPanel = () => {
    ...
    const studentsList = dashboardData.students
      .filter(
        (s) =>
          !currentBatchKey ||
          s.batch === currentBatchKey ||
          s.batch === "Unassigned",
      )
      .map((s) => {
        const sessionRec = s.sessions?.find(
          (ss) => new Date(ss.date).toDateString() === selectedDateString,
        );
        ...
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Live Updates • {checkedInCount} / {studentsList.length} students
              checked in
            </p>
          </div>
          {!dashboardData?.isScheduleClosed && (
            <button
              onClick={() => {
                const studentsInit = studentsList.map((s) => ({
                  studentId: s.studentId,
                  isPresent: s.sessionRec?.isPresent || false,
                  name: s.name,
                  email: s.email,
                }));
                setMarkOfflineModal({
                ...
                return (
                  <tr key={student.studentId} className="hover:bg-gray-50/50">
                    <td
                      ...
                      onClick={() =>
                        setStudentSummaryModal({ open: true, student })
                      }
                    >
                      <p className="font-semibold text-indigo-600 text-sm hover:underline">
                        {student.name}
                      </p>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      ...
                      {isResolved === "override" && (
                        <div className="text-[10px] text-indigo-600 mt-1 font-semibold uppercase tracking-wide">
                          Overridden
                        </div>
                      )}
                    </td>
                        ...
                        <button
                          onClick={() =>
                            setOverrideModal({
                              ...
                              timetableDate:
                                selectedTimetableDate.split("T")[0],
                              isPresent: !isPres,
                              reason: "",
                            })
                          }
                    ...
                    {/* restrict the action button for future sessions override - 03-09-2026 */}
                    <td className="px-6 py-4 text-right">
                      {dashboardData?.isScheduleClosed ? (
                        <span className="text-xs text-gray-400 italic">
                          Locked
                        ...
                        (() => {
                          const isFuture =
                            toLocalDateStr(selectedTimetableDate) >
                            toLocalDateStr(new Date());

                          return isFuture ? (
                            <span
                              className="text-xs text-gray-400 italic"
                              title="Cannot override future sessions"
                            >
                              Future
                            </span>
                          ) : (
      ...
      <div className="bg-gray-50 rounded-2xl shadow-2xl w-full max-w-7xl relative p-6 h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-6 border-b border-gray-200 pb-4 shrink-0 pr-10 gap-6 flex-wrap">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-4 text-sm bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100">
              <div className="flex flex-col">
                ...
                <span className="font-bold text-gray-800">
                  {dashboardData?.minAttendancePercent || 80}%
                </span>
              </div>
                  ...
                  onClick={() =>
                    setSettingsModal({
                      ...
                      onlineMinDurationMins:
                        dashboardData?.onlineMinDurationMins || 0,
            ...
            {dashboardData?.isScheduleClosed &&
              dashboardData?.batchCertSummary?.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap">
                  {dashboardData.batchCertSummary.map((b, i) => (
                    <div
                      key={i}
                      className="bg-white border border-gray-200 rounded-xl px-4 py-2 shadow-sm text-xs min-w-[130px]"
                    >
                      <p
                        className="font-bold text-gray-700 mb-1.5 truncate max-w-[160px]"
                        title={b.batch}
                      >
                        🗂 {b.batch}
                      </p>
                  ...
                  {dashboardData.batchCertSummary.some(
                    (b) => b.eligible > b.certified,
                  ) && (
                    <button
            ...
            {reissueResult && (
              <div
                className={`mt-2 w-full text-xs font-semibold px-3 py-2 rounded-lg ${reissueResult.type === "success" ? "bg-emerald-50 text-emerald-800 border border-emerald-200" : "bg-red-50 text-red-700 border border-red-200"}`}
              >
                ...
                <div className="flex justify-between items-start border-b pb-4 shrink-0">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900">
                      {studentSummaryModal.student.name}
                    </h3>
                    <p className="text-sm text-gray-500">
                      {studentSummaryModal.student.email}
                    </p>
                  </div>
                  <div className="flex flex-col items-end">
                    <div className="flex gap-2 mb-2">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${studentSummaryModal.student.eligible ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                      >
                        {studentSummaryModal.student.eligible
                          ? "Eligible"
                          : "Ineligible"}
                      </span>
                    </div>
                    <span className="text-sm font-bold mt-1 text-gray-700">
                      Attendance: {studentSummaryModal.student.percent}% (
                      {studentSummaryModal.student.attended} sessions)
                    </span>
                  </div>
                </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {studentSummaryModal.student.sessions?.map((sess, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  setOverrideModal({
                                    open: true,
                                    studentId:
                                      studentSummaryModal.student.studentId,
                          ...
                          {/* restrict the future sessions to overridden - 03-09-2026 */}
                          <td className="px-4 py-3 text-right">
                            {dashboardData?.isScheduleClosed ? (
                              <span className="text-xs text-gray-400 italic">
                                Locked
            ...
            <form onSubmit={handleMarkOfflineSubmit}>
              <div className="mb-4">
                ...
                <input
                  type="date"
                  required
                  max={new Date().toISOString().split("T")[0]} // ← blocks future dates 03-09-2026
        ...
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleUpdateSettings}>
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Online Session Min Duration (mins)
                </label>
                {/* <input
                  type="number"
                  min="0"
                  ...
                  onChange={(e) =>
                    setSettingsModal({
                      ...settingsModal,
                      onlineMinDurationMins: Number(e.target.value),
                    })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                />
                ...
                {/* 03-09-2026  for restrict the time within the session duration*/}
                <input
                  type="number"
                  min={0}
                  max={getMaxOnlineDuration()}
                  step={1}
                  value={settingsModal.onlineMinDurationMins}
                  onChange={(e) => {
                    const max = getMaxOnlineDuration();
                    const val = Number(e.target.value) || 0;
                    setSettingsModal({
                      ...settingsModal,
                      onlineMinDurationMins: Math.min(Math.max(0, val), max),
                    });
                  }}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                />
                <p className="text-xs text-gray-500 mt-1">
                  0 = any join counts. Maximum allowed: {getMaxOnlineDuration()}{" "}
                  minutes (shortest session in this schedule)
                </p>
              </div>
              <div className="flex items-center gap-2 mb-6">
                <input
                  ...
                  className="!mt-0 w-4 h-4 text-indigo-600"
                />
                <label
