    ...
    const fetchData = async () => {
      try {
        ...
        const internshipsResponse = await axios.get(
          "http://localhost:3000/api/interns/approved",
        );
        ...
        if (internshipsData.length > 0) {
          try {
            ...
            const internshipIds = internshipsData
              .map((internship) => internship._id)
              .join(",");

            // Fetch counts for these internships
            const countsResponse = await axios.get(`/api/applications/counts`, {
          ...
          {internships.map((internship) => (
            ...
            >
              ...
              <div className="text-gray-500 text-sm mb-3 space-y-1">
                ...
                <p className="flex items-center">
                  {/*Change faDollarSign to faIndianRupee - 15-09-2026 */}
                  <FontAwesomeIcon icon={faIndianRupee} className="mr-2 w-4" />
                  ...
                  {internship.qualifications?.length > 0 ? (
                    internship.qualifications
                      ...
                      .map((qualification, idx) => (
                        <span
                          key={idx}
                          className="text-xs bg-gray-100 text-gray-800 py-1 px-2 rounded-full"
                        >
                          {qualification}
                        </span>
                      ))
